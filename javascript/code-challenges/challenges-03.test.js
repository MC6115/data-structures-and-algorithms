...
const getStatName = (arr, minBaseStat) => {
  const filteredObjects = getBaseStatGreaterThan(arr, minBaseStat);
  return filteredObjects.map(element => element.stat.name);
};
...
// first try, also works

// const evenOddNumericValues = (arr) => {
//   const onlyNum = typeNum(arr);
//   const returnArray = [];
//   onlyNum.map(element=>{ // uses map as if its a for each
//     if(element % 2 !== 0){
//       returnArray.push('odd');
//     } else if(Number.isInteger(element/2)){
