...
const maxInArray = (arr) => {
  let m = arr[0];
  arr.forEach(element => { m = element > m ? element : m; });
  return m;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const hasChildrenEntries = (arr, character) => {
  let hasChildren = false;
  arr.forEach(element => {
    if (Object.entries(element).some(([key, value]) => key === 'name' && value === character)) {
      if (element.children) {
        hasChildren = true;
      }
    }
...
const houseSize = (arr) => {
  ...
  arr.forEach(element=>{
    ...
    sizes.push({house : element.house, members : characterNumber});
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(element => {
    let characterNumber = 0;
    if (!deceasedSpouses.includes(element.name)) {
      characterNumber += 1;
    }
    if (element.children) {
    ...
    if (element.spouse && !deceasedSpouses.includes(element.spouse)) {
      characterNumber += 1;
    }
    survivors.push({ house: element.house, members: characterNumber });
  });
